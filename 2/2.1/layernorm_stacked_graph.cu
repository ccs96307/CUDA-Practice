...
int main() {
    ...
    // Start to capture
    checkCudaError(cudaStreamBeginCapture(stream, cudaStreamCaptureModeGlobal), "cudaStreamBeginCapture");

    // Copy data from host to device
    checkCudaError(cudaMemcpyAsync(d_input, h_input, sizeMat, cudaMemcpyHostToDevice, stream), "cudaMemcpy h_input => d_input");
    checkCudaError(cudaMemcpyAsync(d_gamma, h_gamma, sizeArr, cudaMemcpyHostToDevice, stream), "cudaMemcpy h_gamma => d_gamma");
    checkCudaError(cudaMemcpyAsync(d_beta, h_beta, sizeArr, cudaMemcpyHostToDevice, stream), "cudaMemcpy h_beta => d_beta");

    // Launch LayerNorm Kernel
    dim3 block(NUM_THREADS);
    dim3 grid((B + block.x - 1) / block.x);
    LayerNormKernel<<<grid, block, 0, stream>>>(d_input, d_output, d_gamma, d_beta, epsilon);
    ...
    checkCudaError(cudaMemcpyAsync(h_output, d_output, sizeArr, cudaMemcpyDeviceToHost, stream), "cudaMemcpy d_output => h_output");

    // Stop capture
    checkCudaError(cudaStreamEndCapture(stream, &graph), "cudaStreamEndCapture");

    // Instantiate graph
    checkCudaError(cudaGraphInstantiate(&graphExec, graph, NULL, NULL, 0), "cudaGraphInstantiate graphExec");

    for (int t=0; t<ITERS-1; ++t) {
        checkCudaError(cudaGraphLaunch(graphExec, stream), "cudaGraphLaunch");
    ...
    printf("CUDA Graph - Kernel Spend Time: %f ms\n", ms);
